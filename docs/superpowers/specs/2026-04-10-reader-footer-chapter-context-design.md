...
- current chapter number on the left, 1-indexed
- the start of the current chapter title in the middle
...
- show the current chapter number in the footer
...
- `chapter_number`: 1-indexed chapter position in `chapters.idx`
...
- building a small footer view model with chapter number, title, and progress
...
- left: chapter number, decimal, for example `12`
- middle: chapter title, cropped if needed
- right: progress percent, for example `37%`

Layout rules:

1. Measure the rendered width of the left and right segments first.
2. Reserve at least one character cell of spacing between left and middle.
3. Reserve at least one character cell of spacing between middle and right.
4. The remaining width belongs to the title.
5. If the remaining width is zero or negative, omit the title.
6. If the title width exceeds the available width, hard-crop it to the maximum width that still preserves left, right, and both spaces.
7. If the title fits, render it unchanged.

The chapter number and progress must take priority over the title.
...
4. If chapter metadata is unavailable or invalid, omit chapter number and title.
...
- empty chapter title: show chapter number and progress only
- too little footer width: preserve chapter number and progress, drop title

## Testing

- footer layout test proving left, middle, and right fit with the required spacing
- chapter resolution test proving a page offset maps to the correct 1-indexed chapter and title
- integration test proving the rendered footer includes chapter number, chapter title prefix, and progress on a real cached EPUB fixture

## Recommendation

Implement the feature by resolving chapter footer data in `xteink-fs` and passing plain strings or a small footer model into the existing footer render path. This keeps persistence and chapter lookup close to cache metadata while limiting `xteink-render` to layout concerns.
