...
#[test]
#[ignore = "runtime no longer uses chapter-safe resume checkpoints until exact paginator state is persisted"]
fn resumed_cache_prefix_matches_direct_epub_render_at_page_boundary() {
    let _guard = EPUB_RENDER_TEST_MUTEX
        .lock()
        .expect("render test mutex poisoned");
    let fixture = fixture_dir().join("Decisive - Chip Heath.epub");
    ...
    let mut cached_text = Vec::<u8>::new();
    let mut prefix = Framebuffer::new();
    let prefix_result = prefix
        .build_epub_cache_prefix_with_text_sink_and_cancel(
            VecSource {
                bytes: bytes.clone(),
            },
            0,
    ...
    let target_page = prefix_result.resume_page;

    let mut direct = Framebuffer::new();
    let direct_result = direct.render_epub_page(VecSource { bytes: bytes.clone() }, target_page);
    assert_eq!(direct_result.expect("direct render should succeed"), target_page);

    let mut extension = Framebuffer::new();
    let extension_result = extension
        .extend_epub_cache_prefix_with_text_sink_and_cancel(
            VecSource { bytes },
            target_page,
            prefix_result.resume_page,
            prefix_result.resume_cursor_y,
            prefix_result.next_spine_index,
            |chunk| {
                cached_text.extend_from_slice(chunk.as_bytes());
                Ok(())
            },
        ...
        .expect("cache prefix extension should succeed");

    assert_eq!(extension_result.rendered_page, target_page);
    assert!(!cached_text.is_empty(), "expected cached text to be emitted");
    ...
    assert!(
        diff_bytes <= 64,
        "resumed cached rendering drifted too far from direct EPUB rendering: diff_bytes={} cached_nonwhite={} direct_nonwhite={} prefix_resume_page={} prefix_resume_cursor_y={} prefix_next_spine_index={} extension_rendered_page={} extension_cached_pages={} extension_complete={}",
        diff_bytes,
        cached_nonwhite,
        direct_nonwhite,
        prefix_result.resume_page,
        prefix_result.resume_cursor_y,
        prefix_result.next_spine_index,
        extension_result.rendered_page,
        extension_result.cached_pages,
        extension_result.complete,
    );
}
